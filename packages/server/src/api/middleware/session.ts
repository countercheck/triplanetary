...
const pgStore = {
  async get(key: string) {
    ...
    return typeof row.sess === 'string' ? (JSON.parse(row.sess) as Record<string, unknown>) : row.sess;
  },

  async set(key: string, sess: Record<string, unknown>, maxAge: number | 'session') {
    const expireMs = typeof maxAge === 'number' ? maxAge : 7 * 24 * 60 * 60 * 1000;
...
export function createSessionMiddleware(app: Koa): Koa.Middleware {
  return koaSession(
    {
