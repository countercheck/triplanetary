...
describe('apiRequest', () => {
  ...
  it('throws with status on 401', async () => {
    mswServer.use(
      http.get('/api/unauth', () => HttpResponse.json({ error: 'Unauthorized' }, { status: 401 })),
    );
    const err = await apiRequest('/unauth').catch((e) => e as { status: number; message: string });
  ...
  it('throws with status on 404', async () => {
    mswServer.use(
      http.get('/api/notfound', () => HttpResponse.json({ error: 'Not found' }, { status: 404 })),
    );
    const err = await apiRequest('/notfound').catch((e) => e as { status: number });
    expect(err.status).toBe(404);
  });

  it('throws with status on 500', async () => {
    mswServer.use(
      http.get('/api/boom', () => HttpResponse.json({ error: 'Server error' }, { status: 500 })),
    );
    const err = await apiRequest('/boom').catch((e) => e as { status: number });
...
describe('apiClient helpers', () => {
  ...
  it('apiClient.post delegates to apiRequest POST', async () => {
    mswServer.use(
      http.post('/api/create', async () => HttpResponse.json({ created: true }, { status: 201 })),
    );
    const result = await apiClient.post<{ created: boolean }>('/create', { x: 1 });
